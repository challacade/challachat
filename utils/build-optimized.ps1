#!/usr/bin/env pwsh
# Optimized SEA build - small executable with external assets and dependencies
# No separate runtime folder needed since SEA contains Node.js
...
Write-Host "ChallaChat - Optimized SEA Build" -ForegroundColor Magenta
...
$buildDir = "build/challachat-optimized"
...
process.env.CHALLACHAT_OPTIMIZED = 'true';
...
Compress-Archive -Path "$buildDir/*" -DestinationPath "build/challachat-optimized.zip" -Force

# Calculate sizes
$optimizedSize = (Get-ChildItem "$buildDir" -Recurse | Measure-Object -Property Length -Sum).Sum / 1MB
$zipSize = (Get-Item "build/challachat-optimized.zip").Length / 1MB
...
Write-Host "  - challachat-optimized/ (directory: $([math]::Round($optimizedSize,1)) MB)" -ForegroundColor White
Write-Host "  - challachat-optimized.zip (archive: $([math]::Round($zipSize,1)) MB)" -ForegroundColor White
